...
###################################################################################################
function usage () {
echo "Usage : run_simu.sh runs all the simulations for tb_rom_wrapper_2R."
...
ROM_LATENCY_L=(1 2 3)
KEEP_RD_DATA_L=(0 1)
for rom_latency in "${ROM_LATENCY_L[@]}"; do
  for keep_rd_data in "${KEEP_RD_DATA_L[@]}"; do
    cmd="$run_edalize -m ${module} -t $PROJECT_SIMU_TOOL  -P ROM_LATENCY int $rom_latency -P KEEP_RD_DATA int $keep_rd_data $args"
    echo "==========================================================="
