...
DEPTH_TYPE_L=("DEPTH1_TYPE1" \
              "DEPTH2_TYPE12" \
              "DEPTH1_TYPE3" \
              "DEPTH2_TYPE21" \
              "DEPTH3_TYPE123" \
              "DEPTH1_TYPE2" )
for depth_type in "${DEPTH_TYPE_L[@]}"; do
  if [[ $depth_type =~ DEPTH([0-9]+)_TYPE([1-3]+)$ ]]; then
    depth=${BASH_REMATCH[1]}
    type_fifo=${BASH_REMATCH[2]}
  fi

  cmd="$run_edalize -m ${module} -t $PROJECT_SIMU_TOOL  -P DEPTH int $depth \
                                          -P TYPE_ARRAY int 0x$type_fifo $args"
  echo "==========================================================="
