...
for i in `seq 1 5`; do
    ...
    REGF_SEQ_LOG=$(($RANDOM % 4))
    REGF_SEQ=$((2**$REGF_SEQ_LOG))
    ...
    PEM_PC=$(($RANDOM % ($REGF_SEQ_LOG+1)))
    PEM_PC=$((2**$PEM_PC))
    while [ $PEM_PC -gt $PEM_PC_MAX ] || [ $PEM_PC -gt $REGF_COEF_NB ] ; do
      PEM_PC=$(($RANDOM % ($REGF_SEQ_LOG+1)))
      PEM_PC=$((2**$PEM_PC))
    done
