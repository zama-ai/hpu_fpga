...
###################################################################################################
function usage () {
echo "Usage : run_simu.sh runs all the simulations for tb_ntt_radix_cooley_tukey."
...
USE_MOD_MULT_L=(0 1)
for r in "${R_L[@]}"; do
  for use_mod_mult in "${USE_MOD_MULT_L[@]}"; do
    cmd="$run_edalize -m ${module} -t $PROJECT_SIMU_TOOL  -P R int $r -P USE_MOD_MULT int $use_mod_mult $args"
    echo "==========================================================="
