...
# via mhdma_pkg::REQ_MEMORY_TYPE = "distributed" (REQ_FIFO_DEPTH=64, 2*REG_DATA_W=64b)

# ------------------------------------------------------------------------------
# Distributed RAM (LUTRAM) - small control & command FIFOs
# ------------------------------------------------------------------------------

# Master: notify retry buffer (depth=REQ_FIFO_DEPTH=64, ~60b)
...
# Slave: read request command queue (depth=RREQ_CMD_DEPTH=64, $bits(command_t)=96b)
...
# Decoder: RX command queue (depth=RX_FIFO_DEPTH=128, $bits(command_t)=96b, ~12Kb)
