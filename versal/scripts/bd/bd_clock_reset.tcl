...
proc create_hier_cell_clock_reset { parentCell nameHier } {
  ...
  set MHDMA_FREERUN_FREQ $_nsp_hpu::MHDMA_FREERUN_FREQ
  set MRMAC_APB3_FREQ $_nsp_hpu::MRMAC_APB3_FREQ
  ...
  # Create instance: mhdma_clk_wiz, and set properties
  set mhdma_clk_wiz [ create_bd_cell -type ip -vlnv xilinx.com:ip:clk_wizard:1.0 mhdma_clk_wiz ]
  # we need a stable freerun clock and a qsfp clock
  set mhdma_freq "${MHDMA_FREERUN_FREQ},${MHDMA_FREQ},${MRMAC_APB3_FREQ}"
  set_property -dict [list \
    CONFIG.CLKOUT_DRIVES {No_buffer,No_buffer,No_buffer} \
    CONFIG.CLKOUT_REQUESTED_OUT_FREQUENCY $mhdma_freq \
    CONFIG.CLKOUT_USED {true,true,true} \
    CONFIG.PRIM_SOURCE {No_buffer} \
    CONFIG.USE_DYN_RECONFIG {false} \
    CONFIG.USE_LOCKED {true} \
  ...
  # Create instance: mhdma_freerun_psr, and set properties
  ...
  # Create port connections
  connect_bd_net -net clk_freerun_1 [get_bd_pins clk_freerun] [get_bd_pins usr_clk_wiz/clk_in1] [get_bd_pins mhdma_clk_wiz/clk_in1]
