...
just --set OUTDIR $OUTDIR build $TOP_NAME syn "-F TOP_TOP $TOP_TOP -F PEP_MSPLIT $PEP_MSPLIT -F PEP_BATCH $PEP_BATCH -F TOP_PCMAX $TOP_PCMAX -F TOP_PC $TOP_PC -F APPLICATION $APPLICATION -F NTT_MOD $NTT_MOD -F NTT_CORE_ARCH $NTT_CORE_ARCH -F NTT_CORE_R $NTT_CORE_R -F NTT_CORE_PSI $NTT_CORE_PSI -F NTT_CORE_RDX_CUT $NTT_CORE_RDX_CUT -F NTT_CORE_DIV $NTT_CORE_DIV -F BSK_SLOT $BSK_SLOT -F BSK_CUT $BSK_CUT -F KSK_SLOT $KSK_SLOT -F KSK_CUT $KSK_CUT -F KSLB $KSLB -F HPU_PART $HPU_PART -F AXI_DATA_W $AXI_DATA_W -F REGF_STRUCT $REGF_STRUCT" "P:INTER_PART_PIPE=${INTER_PART_PIPE} D:MEMORY_FILE_PATH=\\\"${PROJECT_DIR}/hw/\\\""| tee build_out_psi16.log
