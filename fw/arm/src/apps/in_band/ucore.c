...
uint32_t parse_iop(
  ...
  printf("IOP mapping 0 %d:%d 1 %d:%d 2 %d:%d\n",
	mapping->mapping.used_0,
	mapping->mapping.virt_0,
	mapping->mapping.used_1,
	mapping->mapping.virt_1,
	mapping->mapping.used_2,
	mapping->mapping.virt_2);
  ...
  do {
    ...
    // Fill current slot
    dst->operand[dst_pos].cid_ofst = opaddr->opaddr.base_cid;
    dst->operand[dst_pos].len = operand->operand.vec_size +1;
    dst->operand[dst_pos].iid = operand->operand.iid;
    dst->operand[dst_pos].pos = operand->operand.pos;
    printf("DST iid %d pos %d len %d cid_ofst %d\n",
	dst->operand[dst_pos].iid,
	dst->operand[dst_pos].pos,
        dst->operand[dst_pos].len,
  ...
  do {
    ...
    // Fill current slot
    src->operand[src_pos].cid_ofst = opaddr->opaddr.base_cid;
    src->operand[src_pos].len = operand->operand.vec_size +1;
    src->operand[src_pos].iid = operand->operand.iid;
    src->operand[src_pos].pos = operand->operand.pos;
    printf("SRC iid %d pos %d len %d cid_ofst %d\n",
	src->operand[src_pos].iid,
	src->operand[src_pos].pos,
        src->operand[src_pos].len,
...
uint32_t get_lookup(IOpHeader_t header, Lookup_t* lookup, uint8_t hpu_id){
  ...
  if ( (entry < 0x20000) ||
       (entry > 0x4000000) ) {
    PLL_ERR("get_lookup", "entry offset is wrong %lx (should be in range 0x20000-0x4000000)", entry);
    lookup->len = 0;
    lookup->ptr = NULL;
    return 1;
  }
