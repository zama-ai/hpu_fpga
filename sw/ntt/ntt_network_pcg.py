        ...
        for stg_iter in range(STG_ITER_NB):
            ...
            for p in range(PSI):
                ...
                for r in range(R):
                    ...
                    next_p = reverse_order(next_p_tmp, R, PSI_W, 0)
                    next_stg_iter = next_node_id & STG_ITER_MASK
                    next_stg_iter = reverse_order(next_stg_iter, R, STG_ITER_W, 0)
            ...
            if (VERBOSE):
                ...
                for next_p in range(PSI):
                    for next_r in range(R):
                        #rev_p = reverse_order(next_p, R, PSI_W, 0)
...
if __name__ == "__main__":
    ...
    for stg in range(S-1):
        ...
        node_idx_1 = 1
        node_id_1 = reverse_order(node_idx_1, R, S-1, delta)
        next_node_id_1 = (node_id_1 << R_W) & NODE_ID_MASK
        next_bu_id_1 = next_node_id_1 >> (STG_ITER_W*R_W)
        next_bu_idx_1 = reverse_order(next_bu_id_1, R, PSI_W, 0)
        if (((next_bu_idx_1%PSI) == 0)): # 0 is the BU idx of the first next BU
            set_nb = 1
        else:
            set_nb = PSI // next_bu_idx_1
        if (VERBOSE):
            print(">> set_nb={:0d} next_bu_idx_1={:0d}".format(set_nb, next_bu_idx_1))
        ...
        node_id_stg_iter_1 = reverse_order(node_idx_stg_iter_1, R, S-1, delta)
        next_node_id_stg_iter_1 = (node_id_stg_iter_1 << R_W) & NODE_ID_MASK
        next_bu_id_stg_iter_1 = next_node_id_stg_iter_1 >> (STG_ITER_W*R_W)
        next_bu_idx_stg_iter_1 = reverse_order(next_bu_id_stg_iter_1, R, PSI_W, 0)
        ...
        for stg_iter in range(STG_ITER_NB):
            ...
            for p in range(PSI):
                ...
                for r in range(R):
                    ...
                    next_stg_iter     = reverse_order(next_stg_iter_tmp, R, STG_ITER_W, 0)
